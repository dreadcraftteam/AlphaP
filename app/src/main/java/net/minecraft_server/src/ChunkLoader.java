...
public class ChunkLoader implements IChunkLoader {
	...
	public Chunk loadChunk(World world, int i, int j) throws IOException {
		...
		if (file != null && file.exists()) {
			try {
				...
				NBTTagCompound nbttagcompound = CompressedStreamTools.func_770_a(exception);
		...
		if (file != null && file.exists()) {
			try {
				...
				if (!cube.isAtLocation(x, y, z)) {
					System.out.println("Cube file at " + x + "," + y + "," + z
							+ " is in the wrong location; relocating. (Expected " + x + ", " + z + ", got "
	...
	public static ChunkCube loadCubeIntoWorldFromCompound(World world, NBTTagCompound cubeTag, int x, int y, int z) {
		...
		if (yStored != y) {
			System.out.println("Cube file at " + x + "," + y + "," + z
					+ " is in the wrong location; relocating. (Expected yPosition: " + y + ", got " + yStored + ")");
		}
	...
	public static Chunk loadChunkIntoWorldFromCompound(World world, NBTTagCompound nbttagcompound, int yStart,
			int yEnd) {
		yStart += 2047;
		yEnd += 2047;
