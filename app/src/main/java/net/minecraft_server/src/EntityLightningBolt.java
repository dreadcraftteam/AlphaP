...
public class EntityLightningBolt extends EntityWeatherEffect {
	...
	public EntityLightningBolt(World world1, double d2, double d4, double d6) {
		...
		if (world1.difficultySetting >= 2 && world1.doChunksNearChunkExist(MathHelper.floor_double(d2),
				MathHelper.floor_double(d4), MathHelper.floor_double(d6), 10)) {
	...
	public void onUpdate() {
		...
		if (this.field_27018_b < 0) {
			if (this.field_27020_c == 0) {
				...
				this.field_27019_a = this.rand.nextLong();
				if (this.worldObj.doChunksNearChunkExist(MathHelper.floor_double(this.posX),
					...
					if (this.worldObj.getBlockId(i1, i2, i3) == 0
							&& Block.fire.canPlaceBlockAt(this.worldObj, i1, i2, i3)) {
		...
		if (this.field_27018_b >= 0) {
			double d6 = 3.0D;
			List list7 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,
					AxisAlignedBB.getBoundingBoxFromPool(this.posX - d6, this.posY - d6, this.posZ - d6, this.posX + d6,
							this.posY + 6.0D + d6, this.posZ + d6));
