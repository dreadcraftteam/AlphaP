...
public class NetLoginHandler extends NetHandler {
	...
	public void doLogin(Packet1Login packet1Login1) {
		...
		if (entityPlayerMP2 != null) {
			...
			logger.info(this.getUserAndIPString() + " logged in with entity id " + entityPlayerMP2.entityId + " at ("
					+ entityPlayerMP2.posX + ", " + entityPlayerMP2.posY + ", " + entityPlayerMP2.posZ + ")");
			...
			netServerHandler5.sendPacket(new Packet1Login("", entityPlayerMP2.entityId, worldServer3.getRandomSeed(),
					(byte) worldServer3.worldProvider.worldType));
			netServerHandler5.sendPacket(
					new Packet6SpawnPosition(chunkCoordinates4.posX, chunkCoordinates4.posY, chunkCoordinates4.posZ));
			this.mcServer.configManager.func_28170_a(entityPlayerMP2, worldServer3);
			this.mcServer.configManager.sendPacketToAllPlayers(
					new Packet3Chat("\u00a7e" + entityPlayerMP2.username + " joined the game."));
			this.mcServer.configManager.playerLoggedIn(entityPlayerMP2);
			netServerHandler5.teleportTo(entityPlayerMP2.posX, entityPlayerMP2.posY, entityPlayerMP2.posZ,
					entityPlayerMP2.rotationYaw, entityPlayerMP2.rotationPitch);
	...
	public String getUserAndIPString() {
		return this.username != null ? this.username + " [" + this.netManager.getRemoteAddress().toString() + "]"
				: this.netManager.getRemoteAddress().toString();
	}
