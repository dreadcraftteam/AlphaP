...
public class WorldGenBigTree extends WorldGenerator {
				...
				if (f8 < 0.0F) {
					...
					for (double d9 = 0.5D; i7 < i1; ++i7) {
						...
						if (this.func_427_a(i17, i18) == -1) {
							...
							double d20 = Math.sqrt(Math.pow((double) Math.abs(this.basePos[0] - i17[0]), 2.0D)
									+ Math.pow((double) Math.abs(this.basePos[2] - i17[2]), 2.0D));
	...
	void func_426_a(int i1, int i2, int i3, float f4, byte b5, int i6) {
		...
		label32: for (i11[b5] = i10[b5]; i12 <= i7; ++i12) {
			i11[b8] = i10[b8] + i12;
			i13 = -i7;
			...
			while (true) {
				while (true) {
					...
					double d15 = Math.sqrt(Math.pow((double) Math.abs(i12) + 0.5D, 2.0D)
							+ Math.pow((double) Math.abs(i13) + 0.5D, 2.0D));
					if (d15 > (double) f4) {
						++i13;
					} else {
						i11[b9] = i10[b9] + i13;
