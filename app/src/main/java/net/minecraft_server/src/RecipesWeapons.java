	...
	public void addRecipes(CraftingManager craftingManager1) {
		for (int i2 = 0; i2 < this.recipeItems[0].length; ++i2) {
			...
			for (int i4 = 0; i4 < this.recipeItems.length - 1; ++i4) {
				...
				craftingManager1.addRecipe(new ItemStack(item5),
						new Object[] { this.recipePatterns[i4], '#', Item.stick, 'X', object3 });
			}
		}

		craftingManager1.addRecipe(new ItemStack(Item.bow, 1),
				new Object[] { " #X", "# X", " #X", 'X', Item.silk, '#', Item.stick });
		craftingManager1.addRecipe(new ItemStack(Item.arrow, 4),
