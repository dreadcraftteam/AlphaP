...
public abstract class Entity {
	...
	public void setPosition(double d, double d1, double d2) {
		...
		this.boundingBox.setBounds(d - (double) f, d1 - (double) this.yOffset + (double) this.ySize, d2 - (double) f,
				d + (double) f, d1 - (double) this.yOffset + (double) this.ySize + (double) f1, d2 + (double) f);
	}
		...
		if (this.handleWaterMovement()) {
			if (!this.inWater && !this.firstUpdate) {
				float f = MathHelper.sqrt_double(this.motionX * this.motionX * (double) 0.2F
						+ this.motionY * this.motionY + this.motionZ * this.motionZ * (double) 0.2F) * 0.2F;
				if (f > 1.0F) {
					f = 1.0F;
				}

				this.worldObj.playSoundAtEntity(this, "random.splash", f,
				...
				for (j = 0; (float) j < 1.0F + this.width * 20.0F; ++j) {
					...
					this.worldObj.spawnParticle("bubble", this.posX + (double) f3, (double) (f1 + 1.0F),
							this.posZ + (double) f5, this.motionX,
				...
				for (j = 0; (float) j < 1.0F + this.width * 20.0F; ++j) {
					f3 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
					f5 = (this.rand.nextFloat() * 2.0F - 1.0F) * this.width;
					this.worldObj.spawnParticle("splash", this.posX + (double) f3, (double) (f1 + 1.0F),
	...
	public void moveEntity(double d, double d1, double d2) {
		if (this.noClip) {
			...
			if (flag) {
				double list;
				for (list = 0.05D; d != 0.0D && this.worldObj
						.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(d, -1.0D, 0.0D))
						.size() == 0; d5 = d) {
					if (d < list && d >= -list) {
						d = 0.0D;
					} else if (d > 0.0D) {
				...
				for (; d2 != 0.0D && this.worldObj
						.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(0.0D, -1.0D, d2))
						.size() == 0; d7 = d2) {
					if (d2 < list && d2 >= -list) {
						d2 = 0.0D;
					} else if (d2 > 0.0D) {
			...
			if (this.func_25017_l() && !flag && this.ridingEntity == null) {
				this.distanceWalkedModified = (float) ((double) this.distanceWalkedModified
						+ (double) MathHelper.sqrt_double(d37 * d37 + d12 * d12) * 0.6D);
				...
				if (this.distanceWalkedModified > (float) this.nextStepDistance && i40 > 0) {
					...
					if (this.worldObj.getBlockId(i38, k1 + 1, i39) == Block.snow.blockID) {
						...
						this.worldObj.playSoundAtEntity(this, stepSound43.func_737_c(), stepSound43.getVolume() * 0.15F,
								stepSound43.getPitch());
	...
	public boolean func_27008_Y() {
		return this.inWater || this.worldObj.canLightningStrikeAt(MathHelper.floor_double(this.posX),
	...
	public boolean handleWaterMovement() {
		return this.worldObj.handleMaterialAcceleration(
				this.boundingBox.expand(0.0D, -0.4000000059604645D, 0.0D).getInsetBoundingBox(0.001D, 0.001D, 0.001D),
				Material.water, this);
	}
	...
	public float getEntityBrightness(float f) {
		...
		int k = MathHelper.floor_double(this.posZ);
		if (this.worldObj.checkChunksExist(MathHelper.floor_double(this.boundingBox.minX),
	...
	public void mountEntity(Entity entity) {
		...
		if (entity == null) {
			...
			this.ridingEntity.riddenByEntity = null;
			this.ridingEntity = null;
			this.setLocationAndAngles(entity.posX, entity.boundingBox.minY + (double) entity.height, entity.posZ,
					this.rotationYaw, this.rotationPitch);
		} else {
			if (this.ridingEntity != null) {
				this.ridingEntity.riddenByEntity = null;
