...
public class BlockDoor extends Block {
	...
	public void setBlockBoundsBasedOnState(IBlockAccess iblockaccess, int i, int j, int k) {
		this.func_273_b(this.func_271_d(iblockaccess.getBlockMetadata(i, j, k)));
	}
	...
	public void func_272_a(World world, int i, int j, int k, boolean flag) {
		...
		if ((l & 8) != 0) {
			if (world.getBlockId(i, j - 1, k) == this.blockID) {
				this.func_272_a(world, i, j - 1, k, flag);
			}
			...
			if (flag1 != flag) {
				...
				world.setBlockMetadataWithNotify(i, j, k, l ^ 4);
				world.markBlocksDirty(i, j - 1, k, i, j, k);
				world.func_28101_a((EntityPlayer) null, 1003, i, j, k, 0);
			}
		}
	}
	...
	public void onNeighborBlockChange(World world, int i, int j, int k, int l) {
		...
		if ((i1 & 8) != 0) {
			...
			if (flag) {
				...
				this.func_272_a(world, i, j, k, flag1);
			}
		}
	...
	public int idDropped(int i, Random random) {
		return (i & 8) != 0 ? 0
				: (this.blockMaterial == Material.iron ? Item.doorSteel.shiftedIndex : Item.doorWood.shiftedIndex);
	}
	...
	public boolean canPlaceBlockAt(World world, int i, int j, int k) {
		return world.isBlockNormalCube(i, j - 1, k) && super.canPlaceBlockAt(world, i, j, k)
