...
public class BlockFlowing extends BlockFluid {
	...
	public void updateTick(World world, int i, int j, int k, Random random) {
		...
		if (l > 0) {
			byte aflag = -100;
			this.field_659_a = 0;
			int aflag1 = this.func_307_e(world, i - 1, j, k, aflag);
			aflag1 = this.func_307_e(world, i + 1, j, k, aflag1);
			aflag1 = this.func_307_e(world, i, j, k - 1, aflag1);
			aflag1 = this.func_307_e(world, i, j, k + 1, aflag1);
			k1 = aflag1 + byte0;
			if (k1 >= 8 || aflag1 < 0) {
				k1 = -1;
			...
			if (this.field_659_a >= 2 && this.blockMaterial == Material.water) {
				if (world.getBlockMaterial(i, j - 1, k).isSolid()) {
					k1 = 0;
				} else if (world.getBlockMaterial(i, j - 1, k) == this.blockMaterial
		...
		if (this.func_312_l(world, i, j - 1, k)) {
			...
			boolean[] aflag2 = this.func_4035_j(world, i, j, k);
			k1 = l + byte0;
			if (l >= 8) {
				k1 = 1;
			}
	...
	private int func_4034_a(World world, int i, int j, int k, int l, int i1) {
		...
		for (int k1 = 0; k1 < 4; ++k1) {
			if ((k1 != 0 || i1 != 1) && (k1 != 1 || i1 != 0) && (k1 != 2 || i1 != 3) && (k1 != 3 || i1 != 2)) {
				...
				if (!this.func_309_k(world, l1, j, j2) && (world.getBlockMaterial(l1, j, j2) != this.blockMaterial
						|| world.getBlockMetadata(l1, j, j2) != 0)) {
					if (!this.func_309_k(world, l1, j - 1, j2)) {
						return l;
					}

					if (l < 4) {
						int k2 = this.func_4034_a(world, l1, j, j2, l + 1, k1);
						if (k2 < j1) {
							j1 = k2;
						}
					}
	...
	private boolean[] func_4035_j(World world, int i, int j, int k) {
		...
		for (i1 = 0; i1 < 4; ++i1) {
			...
			if (!this.func_309_k(world, l1, j, j2) && (world.getBlockMaterial(l1, j, j2) != this.blockMaterial
					|| world.getBlockMetadata(l1, j, j2) != 0)) {
				if (!this.func_309_k(world, l1, j - 1, j2)) {
					this.field_660_c[i1] = 0;
				} else {
					this.field_660_c[i1] = this.func_4034_a(world, l1, j, j2, 1, i1);
				}
			}
		}

		i1 = this.field_660_c[0];

		for (l1 = 1; l1 < 4; ++l1) {
			if (this.field_660_c[l1] < i1) {
				i1 = this.field_660_c[l1];
			}
		}
	...
	private boolean func_312_l(World world, int i, int j, int k) {
		...
		return material == this.blockMaterial ? false
				: (material == Material.lava ? false : !this.func_309_k(world, i, j, k));
	}
