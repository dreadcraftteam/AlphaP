...
class RailLogic {
	...
	private void func_591_b() {
		for (int i1 = 0; i1 < this.connectedTracks.size(); ++i1) {
			...
			if (railLogic2 != null && railLogic2.isConnectedTo(this)) {
				this.connectedTracks.set(i1,
						new ChunkPosition(railLogic2.trackX, railLogic2.trackY, railLogic2.trackZ));
			} else {
	...
	private boolean isMinecartTrack(int i1, int i2, int i3) {
		return BlockRail.func_27029_g(this.worldObj, i1, i2, i3) ? true
				: (BlockRail.func_27029_g(this.worldObj, i1, i2 + 1, i3) ? true
						: BlockRail.func_27029_g(this.worldObj, i1, i2 - 1, i3));
	}

	private RailLogic getMinecartTrackLogic(ChunkPosition chunkPosition1) {
		return BlockRail.func_27029_g(this.worldObj, chunkPosition1.x, chunkPosition1.y, chunkPosition1.z)
				? new RailLogic(this.minecartTrack, this.worldObj, chunkPosition1.x, chunkPosition1.y, chunkPosition1.z)
				: (BlockRail.func_27029_g(this.worldObj, chunkPosition1.x, chunkPosition1.y + 1, chunkPosition1.z)
						? new RailLogic(this.minecartTrack, this.worldObj, chunkPosition1.x, chunkPosition1.y + 1,
								chunkPosition1.z)
						: (BlockRail.func_27029_g(this.worldObj, chunkPosition1.x, chunkPosition1.y - 1,
								chunkPosition1.z)
										? new RailLogic(this.minecartTrack, this.worldObj, chunkPosition1.x,
												chunkPosition1.y - 1, chunkPosition1.z)
										: null));
	}
