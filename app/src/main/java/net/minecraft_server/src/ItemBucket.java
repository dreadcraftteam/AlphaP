...
public class ItemBucket extends Item {
	...
	public ItemStack onItemRightClick(ItemStack itemStack1, World world2, EntityPlayer entityPlayer3) {
		float f4 = 1.0F;
		float f5 = entityPlayer3.prevRotationPitch
				+ (entityPlayer3.rotationPitch - entityPlayer3.prevRotationPitch) * f4;
		...
		double d9 = entityPlayer3.prevPosY + (entityPlayer3.posY - entityPlayer3.prevPosY) * (double) f4 + 1.62D
				- (double) entityPlayer3.yOffset;
		...
		if (movingObjectPosition24 == null) {
			...
			if (movingObjectPosition24.typeOfHit == EnumMovingObjectType.TILE) {
				...
				if (this.isFull == 0) {
					if (world2.getBlockMaterial(i25, i26, i27) == Material.water
							&& world2.getBlockMetadata(i25, i26, i27) == 0) {
						world2.setBlockWithNotify(i25, i26, i27, 0);
						return new ItemStack(Item.bucketWater);
					}

					if (world2.getBlockMaterial(i25, i26, i27) == Material.lava
							&& world2.getBlockMetadata(i25, i26, i27) == 0) {
						world2.setBlockWithNotify(i25, i26, i27, 0);
					...
					if (world2.isAirBlock(i25, i26, i27) || !world2.getBlockMaterial(i25, i26, i27).isSolid()) {
						if (world2.worldProvider.isHellWorld && this.isFull == Block.waterMoving.blockID) {
							world2.playSoundEffect(d7 + 0.5D, d9 + 0.5D, d11 + 0.5D, "random.fizz", 0.5F,
									2.6F + (world2.rand.nextFloat() - world2.rand.nextFloat()) * 0.8F);

							for (int i28 = 0; i28 < 8; ++i28) {
								world2.spawnParticle("largesmoke", (double) i25 + Math.random(),
										(double) i26 + Math.random(), (double) i27 + Math.random(), 0.0D, 0.0D, 0.0D);
							}
						} else {
