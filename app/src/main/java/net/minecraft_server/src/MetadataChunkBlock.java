...
public class MetadataChunkBlock {
	...
	public boolean func_692_a(int i, int j, int k, int l, int i1, int j1) {
		if (i >= this.xStart && j >= this.yStart && k >= this.zStart && l <= this.xEnd && i1 <= this.yEnd
				&& j1 <= this.zEnd) {
			return true;
		} else {
			byte k1 = 1;
			if (i >= this.xStart - k1 && j >= this.yStart - k1 && k >= this.zStart - k1 && l <= this.xEnd + k1
					&& i1 <= this.yEnd + k1 && j1 <= this.zEnd + k1) {
