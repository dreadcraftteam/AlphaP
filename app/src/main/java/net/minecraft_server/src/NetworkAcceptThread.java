		...
		while (this.field_985_b.field_973_b) {
			try {
				...
				if (socket2 != null) {
					...
					if (hashMap1.containsKey(inetAddress3) && !"127.0.0.1".equals(inetAddress3.getHostAddress())
							&& System.currentTimeMillis() - ((Long) hashMap1.get(inetAddress3)).longValue() < 5000L) {
						hashMap1.put(inetAddress3, System.currentTimeMillis());
						socket2.close();
					} else {
						hashMap1.put(inetAddress3, System.currentTimeMillis());
						NetLoginHandler netLoginHandler4 = new NetLoginHandler(this.mcServer, socket2,
								"Connection #" + NetworkListenThread.func_712_b(this.field_985_b));
