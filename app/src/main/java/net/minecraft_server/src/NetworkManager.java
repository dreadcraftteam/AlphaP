...
public class NetworkManager {
	...
	private boolean sendPacket() {
		...
		try {
			...
			Packet packet2;
			Object object3;
			if (!this.dataPackets.isEmpty() && (this.chunkDataSendCounter == 0 || System.currentTimeMillis()
					- ((Packet) this.dataPackets.get(0)).creationTimeMillis >= (long) this.chunkDataSendCounter)) {
			...
			if (this.field_20175_w-- <= 0 && !this.chunkDataPackets.isEmpty()
					&& (this.chunkDataSendCounter == 0 || System.currentTimeMillis() - ((Packet) this.chunkDataPackets
							.get(0)).creationTimeMillis >= (long) this.chunkDataSendCounter)) {
				object3 = this.sendQueueLock;
				synchronized (this.sendQueueLock) {
	...
	private void onNetworkError(Exception exception1) {
		...
		this.networkShutdown("disconnect.genericReason",
				new Object[] { "Internal exception: " + exception1.toString() });
	}
	...
	static void func_30007_a(NetworkManager networkManager0, Exception exception1) {
