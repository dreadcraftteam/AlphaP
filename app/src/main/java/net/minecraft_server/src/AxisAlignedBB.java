...
public class AxisAlignedBB {
	...
	public static AxisAlignedBB getBoundingBoxFromPool(double d0, double d2, double d4, double d6, double d8,
			double d10) {
	...
	public AxisAlignedBB getOffsetBoundingBox(double d1, double d3, double d5) {
		return getBoundingBoxFromPool(this.minX + d1, this.minY + d3, this.minZ + d5, this.maxX + d1, this.maxY + d3,
				this.maxZ + d5);
	}
	...
	public boolean intersectsWith(AxisAlignedBB axisAlignedBB1) {
		return axisAlignedBB1.maxX > this.minX && axisAlignedBB1.minX < this.maxX
				? (axisAlignedBB1.maxY > this.minY && axisAlignedBB1.minY < this.maxY
						? axisAlignedBB1.maxZ > this.minZ && axisAlignedBB1.minZ < this.maxZ
						: false)
				: false;
	}
	...
	public boolean isVecInXYZ(Vec3D vec3D1) {
		return vec3D1.xCoord > this.minX && vec3D1.xCoord < this.maxX
				? (vec3D1.yCoord > this.minY && vec3D1.yCoord < this.maxY
						? vec3D1.zCoord > this.minZ && vec3D1.zCoord < this.maxZ
						: false)
				: false;
	}
	...
	public MovingObjectPosition func_706_a(Vec3D vec3D1, Vec3D vec3D2) {
	...
	private boolean isVecInYZ(Vec3D vec3D1) {
		return vec3D1 == null ? false
				: vec3D1.yCoord >= this.minY && vec3D1.yCoord <= this.maxY && vec3D1.zCoord >= this.minZ
						&& vec3D1.zCoord <= this.maxZ;
	}

	private boolean isVecInXZ(Vec3D vec3D1) {
		return vec3D1 == null ? false
				: vec3D1.xCoord >= this.minX && vec3D1.xCoord <= this.maxX && vec3D1.zCoord >= this.minZ
						&& vec3D1.zCoord <= this.maxZ;
	}

	private boolean isVecInXY(Vec3D vec3D1) {
		return vec3D1 == null ? false
				: vec3D1.xCoord >= this.minX && vec3D1.xCoord <= this.maxX && vec3D1.yCoord >= this.minY
						&& vec3D1.yCoord <= this.maxY;
	}
