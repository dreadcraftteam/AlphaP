...
public class ConsoleCommandHandler {
						...
						if (string2.toLowerCase().startsWith("ban ")) {
							...
							if (string2.toLowerCase().startsWith("kick ")) {
								...
								for (i8 = 0; i8 < serverConfigurationManager5.playerEntities.size(); ++i8) {
									EntityPlayerMP entityPlayerMP9 = (EntityPlayerMP) serverConfigurationManager5.playerEntities
											.get(i8);
								...
								if (string2.toLowerCase().startsWith("tp ")) {
									...
									if (string18.length == 3) {
										...
										if (entityPlayerMP14 == null) {
											...
											iCommandListener3.log("User " + string18[1] + " and " + string18[2]
													+ " are in different dimensions. No tp.");
										} else {
											entityPlayerMP14.playerNetServerHandler.teleportTo(entityPlayerMP15.posX,
													entityPlayerMP15.posY, entityPlayerMP15.posZ,
													entityPlayerMP15.rotationYaw, entityPlayerMP15.rotationPitch);
											this.sendNoticeToOps(string4,
													"Teleporting " + string18[1] + " to " + string18[2] + ".");
										}
									} else {
									...
									if (string2.toLowerCase().startsWith("give ")) {
										...
										if (entityPlayerMP15 != null) {
											try {
												...
												if (Item.itemsList[i17] != null) {
													this.sendNoticeToOps(string4,
															"Giving " + entityPlayerMP15.username + " some " + i17);
													int i10 = 1;
													if (string18.length > 3) {
										...
										try {
											...
											if ("add".equalsIgnoreCase(string16)) {
												for (i17 = 0; i17 < this.minecraftServer.worldMngr.length; ++i17) {
													...
													worldServer19
															.func_32005_b(worldServer19.getWorldTime() + (long) i8);
												}
										...
										serverConfigurationManager5
												.sendPacketToAllPlayers(new Packet3Chat("\u00a7d[Server] " + string2));
										...
										if (string18.length >= 3) {
											...
											if (!serverConfigurationManager5.sendPacketToPlayer(string18[1],
													new Packet3Chat(string2))) {
	...
	private void printHelp(ICommandListener iCommandListener1) {
		...
		iCommandListener1
				.log("   pardon-ip <ip>            pardons a banned IP address so that they can connect again");
