...
public class Pathfinder {
	...
	public PathEntity createEntityPathTo(Entity entity1, int i2, int i3, int i4, float f5) {
		return this.createEntityPathTo(entity1, (double) ((float) i2 + 0.5F), (double) ((float) i3 + 0.5F),
				(double) ((float) i4 + 0.5F), f5);
	}

	private PathEntity createEntityPathTo(Entity entity1, double d2, double d4, double d6, float f8) {
		this.path.clearPath();
		this.pointMap.clearMap();
		PathPoint pathPoint9 = this.openPoint(MathHelper.floor_double(entity1.boundingBox.minX),
				MathHelper.floor_double(entity1.boundingBox.minY), MathHelper.floor_double(entity1.boundingBox.minZ));
		PathPoint pathPoint10 = this.openPoint(MathHelper.floor_double(d2 - (double) (entity1.width / 2.0F)),
				MathHelper.floor_double(d4), MathHelper.floor_double(d6 - (double) (entity1.width / 2.0F)));
		PathPoint pathPoint11 = new PathPoint(MathHelper.floor_float(entity1.width + 1.0F),
				MathHelper.floor_float(entity1.height + 1.0F), MathHelper.floor_float(entity1.width + 1.0F));
		PathEntity pathEntity12 = this.addToPath(entity1, pathPoint9, pathPoint10, pathPoint11, f8);
		return pathEntity12;
	}

	private PathEntity addToPath(Entity entity1, PathPoint pathPoint2, PathPoint pathPoint3, PathPoint pathPoint4,
			float f5) {
			...
			for (int i9 = 0; i9 < i8; ++i9) {
				...
				if (!pathPoint10.isAssigned() || f11 < pathPoint10.totalPathDistance) {
					...
					if (pathPoint10.isAssigned()) {
						this.path.changeDistance(pathPoint10,
								pathPoint10.totalPathDistance + pathPoint10.distanceToNext);
					} else {
	...
	private int findPathOptions(Entity entity1, PathPoint pathPoint2, PathPoint pathPoint3, PathPoint pathPoint4,
			float f5) {
		int i6 = 0;
		byte b7 = 0;
		if (this.getVerticalOffset(entity1, pathPoint2.xCoord, pathPoint2.yCoord + 1, pathPoint2.zCoord,
				pathPoint3) == 1) {
			b7 = 1;
		}

		PathPoint pathPoint8 = this.getSafePoint(entity1, pathPoint2.xCoord, pathPoint2.yCoord, pathPoint2.zCoord + 1,
				pathPoint3, b7);
		PathPoint pathPoint9 = this.getSafePoint(entity1, pathPoint2.xCoord - 1, pathPoint2.yCoord, pathPoint2.zCoord,
				pathPoint3, b7);
		PathPoint pathPoint10 = this.getSafePoint(entity1, pathPoint2.xCoord + 1, pathPoint2.yCoord, pathPoint2.zCoord,
				pathPoint3, b7);
		PathPoint pathPoint11 = this.getSafePoint(entity1, pathPoint2.xCoord, pathPoint2.yCoord, pathPoint2.zCoord - 1,
				pathPoint3, b7);
	...
	private final PathPoint openPoint(int i1, int i2, int i3) {
		int i4 = PathPoint.func_22203_a(i1, i2, i3);
