...
public class WorldGenDungeons extends WorldGenerator {
		...
		for (tileentitymobspawner = i - l - 1; tileentitymobspawner <= i + l + 1; ++tileentitymobspawner) {
			for (l2 = j - 1; l2 <= j + byte0 + 1; ++l2) {
				for (k3 = k - i1 - 1; k3 <= k + i1 + 1; ++k3) {
					...
					if ((tileentitymobspawner == i - l - 1 || tileentitymobspawner == i + l + 1 || k3 == k - i1 - 1
							|| k3 == k + i1 + 1) && l2 == j && world.isAirBlock(tileentitymobspawner, l2, k3)
							&& world.isAirBlock(tileentitymobspawner, l2 + 1, k3)) {
						++j1;
					}
				}
			}
		...
		if ((j1 < 1 || j1 > 5) && (j > 0 || random.nextInt(8) != 0)) {
			...
			for (tileentitymobspawner = i - l - 1; tileentitymobspawner <= i + l + 1; ++tileentitymobspawner) {
				for (l2 = j + byte0; l2 >= j - 1; --l2) {
					for (k3 = k - i1 - 1; k3 <= k + i1 + 1; ++k3) {
						if (tileentitymobspawner != i - l - 1 && l2 != j - 1 && k3 != k - i1 - 1
								&& tileentitymobspawner != i + l + 1 && l2 != j + byte0 + 1 && k3 != k + i1 + 1) {
			...
			label113: for (tileentitymobspawner = 0; tileentitymobspawner < 2; ++tileentitymobspawner) {
				for (l2 = 0; l2 < 3; ++l2) {
	...
	private ItemStack pickCheckLootItem(Random random) {
		...
		return i == 0 ? new ItemStack(Item.saddle)
				: (i == 1 ? new ItemStack(Item.ingotIron, random.nextInt(4) + 1)
						: (i == 2 ? new ItemStack(Item.bread)
								: (i == 3 ? new ItemStack(Item.wheat, random.nextInt(4) + 1)
										: (i == 4 ? new ItemStack(Item.gunpowder, random.nextInt(4) + 1)
												: (i == 5 ? new ItemStack(Item.silk, random.nextInt(4) + 1)
														: (i == 6 ? new ItemStack(Item.bucketEmpty)
																: (i == 7 && random.nextInt(100) == 0
																		? new ItemStack(Item.appleGold)
																		: (i == 8 && random.nextInt(2) == 0
																				? new ItemStack(Item.redstone,
																						random.nextInt(4) + 1)
																				: (i == 9 && random.nextInt(10) == 0
																						? new ItemStack(
																								Item.itemsList[Item.record13.shiftedIndex
																										+ random.nextInt(
																												2)])
																						: (i == 10 ? new ItemStack(
																								Item.dyePowder, 1, 3)
