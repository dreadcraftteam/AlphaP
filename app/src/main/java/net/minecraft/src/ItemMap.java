...
public class ItemMap extends ItemMapBase {
	...
	public static MapData func_28013_a(short word0, World world) {
		(new StringBuilder()).append("map_").append(word0).toString();
		MapData mapdata = (MapData)world.loadItemData(MapData.class, "map_" + word0);
		if(mapdata == null) {
	...
	public void func_28011_a(World world, Entity entity, MapData mapdata) {
		if(world.worldProvider.worldType == mapdata.field_28178_d) {
			...
			for(int k1 = l - j1 + 1; k1 < l + j1; ++k1) {
				if((k1 & 15) == (mapdata.field_28175_g & 15)) {
					int l1 = 255;
					int i2 = 0;
					double d = 0.0D;
					...
					for(int j2 = i1 - j1 - 1; j2 < i1 + j1; ++j2) {
						if(k1 >= 0 && j2 >= -1 && k1 < c && j2 < c1) {
										...
										if(d2 > 1) {
											...
											while(true) {
												...
												if(d2 > -64) {
													if(l6 == 0) {
														byte0 = false;
													} else if(d2 > 0 && l6 > 0 && Block.blocksList[l6].blockMaterial.materialMapColor == MapColor.airColor) {
														byte0 = false;
													}
												}
												...
												if(byte0) {
													...
													while(true) {
														...
														++l4;
														if(j7 <= 0 || i44 == 0 || !Block.blocksList[i44].blockMaterial.getIsLiquid()) {
															break label175;
														}
													}
												}
							...
							if(i6 > 0) {
								...
								if(mapColor45 == MapColor.waterColor) {
								...
								j7 = mapColor45.colorIndex;
							}

							d = d1;
							if(j2 >= 0 && k2 * k2 + l2 * l2 < j1 * j1 && (!flag || (k1 + j2 & 1) != 0)) {
								byte b46 = mapdata.field_28176_f[k1 + j2 * c];
								...
								if(b46 != b47) {
									...
									mapdata.field_28176_f[k1 + j2 * c] = b47;
								}
							}
						}
					}
	...
	public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		if(!world.multiplayerWorld) {
			MapData mapdata = this.func_28012_a(itemstack, world);
			...
			if(flag) {
				this.func_28011_a(world, entity, mapdata);
			}
