...
public class ChunkCube {
	...
	public int setChunkData(byte[] adata, int x1, int y1, int z1, int x2, int y2, int z2, int mOffset) {
		...
		for(x3 = x1; x3 < x2; ++x3) {
			for(z3 = z1; z3 < z2; ++z3) {
				dataOffset = x3 << 8 | z3 << 4 | y1;
				offsetChange = y2 - y1;
				System.arraycopy(adata, mOffset, this.blocks, dataOffset, offsetChange);
				mOffset += offsetChange;
			}
		}
		...
		for(x3 = x1; x3 < x2; ++x3) {
			for(z3 = z1; z3 < z2; ++z3) {
				dataOffset = (x3 << 8 | z3 << 4 | y1) >> 1;
				offsetChange = (y2 - y1) / 2;
				System.arraycopy(adata, mOffset, this.data.data, dataOffset, offsetChange);
				mOffset += offsetChange;
			}
		}
		...
		for(x3 = x1; x3 < x2; ++x3) {
			for(z3 = z1; z3 < z2; ++z3) {
				dataOffset = (x3 << 8 | z3 << 4 | y1) >> 1;
				offsetChange = (y2 - y1) / 2;
				System.arraycopy(adata, mOffset, this.blocklightMap.data, dataOffset, offsetChange);
				mOffset += offsetChange;
			}
		}
		...
		for(x3 = x1; x3 < x2; ++x3) {
			for(z3 = z1; z3 < z2; ++z3) {
				dataOffset = (x3 << 8 | z3 << 4 | y1) >> 1;
				offsetChange = (y2 - y1) / 2;
				System.arraycopy(adata, mOffset, this.skylightMap.data, dataOffset, offsetChange);
				mOffset += offsetChange;
			}
		}
	...
	private ChunkCube getCube(int xDist, int yDist, int zDist) {
		return this.chunk.worldObj.cubeExists(this.chunk.xPosition + xDist, this.yPosition + yDist, this.chunk.zPosition + zDist) ? this.chunk.worldObj.getChunkFromChunkCoords(this.chunk.xPosition + xDist, this.yPosition + yDist, this.chunk.zPosition + zDist) : null;
	}
	...
	public void regenSkylightMap() {
		if(!this.chunk.worldObj.worldProvider.hasNoSky) {
			...
			if(this.chunk.worldObj.cubeExists(x + 1, y, z)) {
				this.chunk.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xBl + 15, yBl, zBl, xBl + 16, yBl + 15, zBl + 15);
			}

			if(this.chunk.worldObj.cubeExists(x - 1, y, z)) {
				this.chunk.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xBl - 1, yBl, zBl, xBl, yBl + 15, zBl + 15);
			}

			if(this.chunk.worldObj.cubeExists(x, y + 1, z)) {
				this.chunk.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xBl, yBl + 15, zBl, xBl + 15, yBl + 16, zBl + 15);
			}

			if(this.chunk.worldObj.cubeExists(x, y - 1, z)) {
				this.chunk.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xBl, yBl - 1, zBl, xBl + 16, yBl, zBl + 16);
			}

			if(this.chunk.worldObj.cubeExists(x, y, z + 1)) {
				this.chunk.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xBl, yBl, zBl + 15, xBl + 16, yBl + 16, zBl + 16);
			}

			if(this.chunk.worldObj.cubeExists(x, y, z - 1)) {
				this.chunk.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xBl, yBl, zBl - 1, xBl + 16, yBl + 16, zBl);
			}
	...
	public void regenOnLoad() {
		this.regenHeightMap();
		if(!this.chunk.worldObj.multiplayerWorld) {
