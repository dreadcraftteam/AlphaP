...
public class MapGenCaves extends MapGenBase {
	...
	protected void genCave(int i, int j, byte[] abyte0, double d, double d1, double d2, float f, float f1, float f2, int k, int l, double d3) {
		...
		if(l <= 0) {
			int flag = this.field_1306_a * 16 - 16;
		...
		for(boolean flag1 = random.nextInt(6) == 0; k < l; ++k) {
			...
			if(!z60 && k == j1 && f > 1.0F) {
				this.genCave(i, j, abyte0, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - 1.570796F, f2 / 3.0F, k, l, 1.0D);
				this.genCave(i, j, abyte0, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + 1.570796F, f2 / 3.0F, k, l, 1.0D);
				return;
			}
	...
	protected void func_868_a(World world, int i, int j, int k, int l, byte[] abyte0) {
		...
		for(int j1 = 0; j1 < i1; ++j1) {
			...
			if(this.rand.nextInt(4) == 0) {
				this.func_870_a(k, l, abyte0, d, d1, d2);
	...
	protected void genCave(int xCh, int yCh, int zCh, byte[] data, double x1, double y1, double z1, float f, float f1, float f2, int k, int l, double d3) {
		...
		for(boolean flag1 = random.nextInt(6) == 0; k < l; ++k) {
			...
			f3 += (random.nextFloat() - random.nextFloat()) * random.nextFloat() * 4.0F;
			if(!z63 && k == j1 && f > 1.0F) {
				this.genCave(xCh, yCh, zCh, data, x1, y1, z1, random.nextFloat() * 0.5F + 0.5F, f1 - 1.570796F, f2 / 3.0F, k, l, 1.0D);
				this.genCave(xCh, yCh, zCh, data, x1, y1, z1, random.nextFloat() * 0.5F + 0.5F, f1 + 1.570796F, f2 / 3.0F, k, l, 1.0D);
				return;
			}
			...
			if(z63 || random.nextInt(4) != 0) {
				...
				if(x1 >= xBl - 16.0D - d6 * 2.0D && z1 >= zBl - 16.0D - d6 * 2.0D && x1 <= xBl + 16.0D + d6 * 2.0D && z1 <= zBl + 16.0D + d6 * 2.0D) {
