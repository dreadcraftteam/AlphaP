...
public class MapGenBase {
	...
	public void func_867_a(IChunkProvider ichunkprovider, World world, int i, int j, byte[] abyte0) {
		...
		for(int i1 = i - k; i1 <= i + k; ++i1) {
			for(int j1 = j - k; j1 <= j + k; ++j1) {
				...
				this.func_868_a(world, i1, j1, i, j, abyte0);
			}
		}
	...
	public void generate(IChunkProvider ichunkprovider, World world, int xCh, int yCh, int zCh, byte[] data) {
		...
		this.rand.setSeed((long)xCh * r1 + (long)zCh * r2 ^ world.getRandomSeed() + (long)yCh * r3 ^ world.getRandomSeed() * world.getRandomSeed());

		for(int x1 = xCh - range; x1 <= xCh + range; ++x1) {
			for(int z1 = zCh - range; z1 <= zCh + range; ++z1) {
				for(int y1 = zCh - range; y1 <= zCh + range; ++y1) {
					this.rand.setSeed((long)x1 * r1 + (long)z1 * r2 ^ world.getRandomSeed() + (long)y1 * r3 ^ world.getRandomSeed() * world.getRandomSeed());
