...
public abstract class EntityPlayer extends EntityLiving {
	...
	public void onLivingUpdate() {
		...
		this.field_9328_RRR += (f1 - this.field_9328_RRR) * 0.8F;
		if(this.health > 0) {
	...
	protected void collideWithPlayer(Entity entity) {
	...
	public static ChunkCoordinates func_25060_a(World world, ChunkCoordinates chunkcoordinates) {
		...
		for(int chunkcoordinates1 = -3; chunkcoordinates1 <= 3; chunkcoordinates1 += 6) {
			for(int z = -3; z <= 3; z += 6) {
				for(int y = -3; y <= 3; y += 6) {
					ichunkprovider.prepareCube(chunkcoordinates.x + chunkcoordinates1 >> 4, chunkcoordinates.y + y >> 4, chunkcoordinates.z + z >> 4);
				}
			}
		}
		...
		if(world.getBlockId(chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z) != Block.blockBed.blockID) {
			...
			ChunkCoordinates chunkcoordinates11 = BlockBed.getNearestEmptyChunkCoordinates(world, chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z, 0);
	...
	protected void fall(float f) {
		if(this.worldObj.multiplayerWorld || !this.username.equals("Dreadcraft")) {
			super.fall(f);
		}
