...
public class Chunk {
	...
	public void func_996_c(int i, int j) {
	...
	public void func_1003_g(int x, int y, int z) {
		...
		int lightLevel;
		label109:
		for(xabs = this.cubes.length - 1; xabs >= 0; --xabs) {
		...
		if(height != colHeight) {
			...
			int heightB4;
			for(heightB4 = height; height > -32752 && this.cubes[(height >> 4) + 2047] != null && lightLevel > 0; this.setSkylightMapNibble(x, height, z, lightLevel)) {
				--height;
			...
			while(height > -32752 && this.cubes[(height >> 4) + 2047] != null && Block.lightOpacity[this.getBlockID(x, height - 1, z)] == 0) {
				--height;
			}

			if(height != heightB4) {
				this.worldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xabs - 1, height, i12 - 1, xabs + 1, heightB4, i12 + 1);
			}
	...
	public int getBlockID(int x, int y, int z) {
		int cube = (y >> 4) + 2047;
		return cube >= 0 && cube < this.cubes.length && this.cubes[cube] != null ? this.cubes[cube].getBlockID(x, y & 15, z) & 255 : 0;
	}
	...
	public int getBlockMetadata(int x, int y, int z) {
		int cube = (y >> 4) + 2047;
		return cube >= 0 && cube < this.cubes.length && this.cubes[cube] != null ? this.cubes[cube].getBlockMetadata(x, y & 15, z) : 0;
	}
	...
	public int getSavedLightValue(EnumSkyBlock enumskyblock, int i, int j, int k) {
		int cube = j >> 2051;
		return cube < 0 ? 0 : (cube >= this.cubes.length ? (enumskyblock == EnumSkyBlock.Sky ? 15 : 0) : (enumskyblock == EnumSkyBlock.Sky ? this.getSkylightMapNibble(i, j, k) : (enumskyblock == EnumSkyBlock.Block ? this.getBlocklightMapNibble(i, j, k) : 0)));
	}
	...
	public void onChunkLoad() {
		...
		for(int y = 0; y < this.cubes.length; ++y) {
			if(this.cubes[y] != null) {
				this.worldObj.func_31054_a(this.cubes[y].chunkTileEntityMap.values());
				this.worldObj.func_636_a(this.cubes[y].entities);
			}
		}
	...
	public void onChunkUnload() {
		...
		for(int y = 0; y < this.cubes.length; ++y) {
			if(this.cubes[y] != null) {
				...
				this.worldObj.func_632_b(this.cubes[y].entities);
			}
		}
