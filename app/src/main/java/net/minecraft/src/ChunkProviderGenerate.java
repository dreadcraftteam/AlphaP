...
public class ChunkProviderGenerate implements IChunkProvider {
	...
	public Chunk provideChunk(int x, int z) {
		...
		this.mapGenCaves.func_867_a(this, this.worldObj, x, z, data);
	...
	private double[] func_4061_a(double[] ad, int x, int y, int z, int xSize, int ySize, int zSize) {
		...
		this.field_4182_g = this.noiseGen6.func_4109_a(this.field_4182_g, x, z, xSize, zSize, 1.121D, 1.121D, 0.5D);
		this.field_4181_h = this.noiseGen7.func_4109_a(this.field_4181_h, x, z, xSize, zSize, 200.0D, 200.0D, 0.5D);

		this.field_4185_d = this.noiseGen3.generateNoiseOctaves(this.field_4185_d, (double) x, (double) y, (double) z,
				xSize, ySize, zSize, d / 320.0D, d1 / 3200.0D, d / 320.0D);
		this.field_4184_e = this.noiseGen1.generateNoiseOctaves(this.field_4184_e, (double) x, (double) y, (double) z,
				xSize, ySize, zSize, d, d1, d);
		this.field_4183_f = this.noiseGen2.generateNoiseOctaves(this.field_4183_f, (double) x, (double) y, (double) z,
				xSize, ySize, zSize, d, d1, d);
		int k1 = 0;
		int l1 = 0;
		int i2 = 16 / xSize;
		...
		for (int j2 = 0; j2 < xSize; ++j2) {
			...
			for (int l2 = 0; l2 < zSize; ++l2) {
				...
				for (int j3 = 0; j3 < ySize; ++j3) {
					...
					d8 -= d9;
					if (j3 > ySize - 4) {
						double d13 = (double) ((float) (j3 - (ySize - 4)) / 3.0F);
	...
	public void populate(IChunkProvider ichunkprovider, int xCh, int yCh, int zCh) {
		...
		d = 0.5D;
		k4 = (int) ((this.mobSpawnerNoise.func_806_a((double) xBlock * d, (double) zBlock * d) / 8.0D
		...
		// if(this.rand.nextInt(8) == 0) {
		// byte b30 = 1;
		// if(biomegenbase == BiomeGenBase.forest && this.rand.nextInt(3) != 0) {
		// b30 = 2;
		// }
		...
		// (new WorldGenDeadBush(Block.deadBush.blockID)).generate(this.worldObj,
		// this.rand, x2, i31, x3);
		// }
		// }
